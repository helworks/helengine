...
namespace Helengine.PhysicsComparison {
    /// <summary>
    /// Runs the authored four-box dynamic stack scene through helengine at fixed simulation-step boundaries.
    /// </summary>
    public sealed class HelengineDynamicStackBoxesTraceRunner {
        /// <summary>
        /// Runs the helengine scene and writes its fixed-step traces.
        /// </summary>
        ...
        public IReadOnlyList<PhysicsTraceSample> Run(string outputDirectoryPath, int stepCount, float stepSeconds) {
            ...
            using PhysicsTraceWriter traceWriter = new PhysicsTraceWriter(Path.Combine(outputDirectoryPath, "helengine-dynamic-stack-trace.csv"));
            using HelengineContactTraceWriter contactTraceWriter = new HelengineContactTraceWriter(Path.Combine(outputDirectoryPath, "helengine-dynamic-stack-contacts.csv"));
            ...
            try {
                ...
                for (int stepIndex = 0; stepIndex <= stepCount; stepIndex++) {
                    for (int bodyIndex = 0; bodyIndex < boxEntities.Length; bodyIndex++) {
                        CaptureSample(samples, traceWriter, "helengine-dynamic-stack", stepIndex, stepSeconds, DynamicStackBoxesSceneDefinition.CreateBodyName(bodyIndex), boxEntities[bodyIndex], boxBodies[bodyIndex], previousLinearVelocities[bodyIndex], previousAngularVelocities[bodyIndex]);
        ...
        static string CreateBodyStateName(int bodyStateIndex) {
            ...
            return DynamicStackBoxesSceneDefinition.CreateBodyName(bodyStateIndex - 1);
        }
