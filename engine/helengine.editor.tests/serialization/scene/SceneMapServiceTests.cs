...
namespace helengine.editor.tests.serialization.scene {
    ...
    public sealed class SceneMapServiceTests : IDisposable {
        ...
        /// <summary>
        /// Ensures return-to-menu also resolves the logical menu id when a pointer click activates one clickable host.
        /// </summary>
        [Fact]
        public void Update_WhenReturnToMenuPointerClickIsTriggeredAndMappingExists_LoadsMappedSceneId() {
            WriteSceneAsset("cooked/scenes/DemoDiscMainMenu.hasset", 1u);
            WriteSceneAsset("cooked/scenes/DemoDiscMainMenuDs.hasset", 2u);
            ...
            sceneMapComponent.Mappings.Add("DemoDiscMainMenu", "DemoDiscMainMenuDs");
            AddLoadedScene(core.SceneManager, "Scenes/Persistent.helen", CreateRootEntityWithComponent(sceneMapComponent));
            CreateUiCamera(new float4(0f, 0f, 320f, 240f), 1, 255);

            Entity returnButtonEntity = CreateRootEntity();
            returnButtonEntity.Position = new float3(24f, 24f, 0f);
            returnButtonEntity.LayerMask = 1;
            returnButtonEntity.AddComponent(new RoundedRectComponent {
                Size = new int2(120, 32),
                RenderOrder2D = 220
            });
            returnButtonEntity.AddComponent(new InteractableComponent {
                Size = new int2(120, 32)
            });
            returnButtonEntity.AddComponent(new DemoDiscReturnToMenuRuntimeComponent());

            inputBackend.SetMouseState(CreateReleasedMouseState(40, 40));
            core.Update(1d / 60d);
            inputBackend.SetMouseState(CreatePressedMouseState(40, 40));
            core.Update(1d / 60d);
            inputBackend.SetMouseState(CreateReleasedMouseState(40, 40));
            core.Update(1d / 60d);
        ...
        /// <summary>
        /// Ensures one pointer-enabled return host does not request a second main-menu load after the first one already succeeded.
        /// </summary>
        [Fact]
        public void Update_WhenReturnToMenuPointerClickRepeatsAfterSuccessfulLoad_DoesNotReloadTargetScene() {
            WriteSceneAsset("cooked/scenes/DemoDiscMainMenu.hasset", 1u);
            WriteSceneAsset("cooked/scenes/DemoDiscMainMenuDs.hasset", 2u);
