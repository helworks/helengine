...
namespace helengine.editor.tests {
    ...
    public class EditorPlatformBuildScenePackagerTests : IDisposable {
        ...
        public void Package_WhenSceneContainsPhysics3DBoxBody_PreservesPhysicsComponentRecords() {
                    ...
                    new SceneEntityAsset {
                        ...
                        Components = new[] {
                            new SceneComponentAssetRecord {
                                ...
                                ComponentIndex = 0,
                                Payload = WriteRigidBody3DComponentPayload(0, false)
                            },
                            new SceneComponentAssetRecord {
            ...
            Assert.Equal(0u, packagedScene.Physics3DSceneFeatureFlags);
        }

        /// <summary>
        /// Ensures the default scene packager preserves the current pass-through physics records required by the runtime kinematic-motion pusher.
        /// </summary>
        [Fact]
                    ...
                    new SceneEntityAsset {
                        ...
                        Components = new[] {
                            new SceneComponentAssetRecord {
                                ComponentTypeId = "helengine.RigidBody3DComponent",
                                ComponentIndex = 0,
                                Payload = WriteRigidBody3DComponentPayload(1, false)
                            },
                            new SceneComponentAssetRecord {
                                ComponentTypeId = "helengine.BoxCollider3DComponent",
                                ComponentIndex = 1,
        ...
        /// <summary>
        /// Ensures the default scene packager preserves the current pass-through physics records required by the runtime character controller.
        /// </summary>
        [Fact]
        public void Package_WhenSceneContainsPhysics3DCharacterController_PreservesPhysicsComponentRecords() {
            ...
            SceneAsset sceneAsset = new SceneAsset {
                ...
                RootEntities = new[] {
                    new SceneEntityAsset {
                        ...
                        LocalPosition = new float3(-4f, 0.75f, 0f),
                        LocalScale = new float3(0.9f, 1.5f, 0.9f),
                        LocalOrientation = float4.Identity,
                        Components = new[] {
                            new SceneComponentAssetRecord {
                                ComponentTypeId = "helengine.BoxCollider3DComponent",
                                ComponentIndex = 0,
                                Payload = WriteBoxCollider3DComponentPayload(new float3(0.9f, 1.5f, 0.9f))
                            },
                            new SceneComponentAssetRecord {
                                ComponentTypeId = "helengine.CharacterController3DComponent",
                                ComponentIndex = 1,
                                Payload = WriteCharacterController3DComponentPayload(new float3(1f, 0f, 0f), 3d, 1d, 0.75d, 0.3d)
                            }
                        },
                        Children = Array.Empty<SceneEntityAsset>()
                    }
            ...
            Assert.Contains(packagedRoot.Components, component => string.Equals(component.ComponentTypeId, "helengine.BoxCollider3DComponent", StringComparison.Ordinal));
            Assert.Contains(packagedRoot.Components, component => string.Equals(component.ComponentTypeId, "helengine.CharacterController3DComponent", StringComparison.Ordinal));
            Assert.Equal((uint)PhysicsSceneFeatureFlags3D.CharacterController, packagedScene.Physics3DSceneFeatureFlags);
        }

        /// <summary>
        /// Ensures packaged scenes preserve the trigger capability bit when a boxed trigger volume is authored.
        /// </summary>
        [Fact]
        public void Package_WhenSceneContainsPhysics3DTriggerVolume_StoresPhysicsFeatureFlags() {
                    ...
                    new SceneEntityAsset {
                        ...
                        Components = new[] {
                            ...
                            new SceneComponentAssetRecord {
                                ComponentTypeId = "helengine.BoxCollider3DComponent",
                                ComponentIndex = 1,
                                Payload = WriteBoxCollider3DComponentPayload(new float3(2f, 2f, 2f), true)
                            }
                        },
                        Children = Array.Empty<SceneEntityAsset>()
                    }
    ...
    /// </summary>
    /// <param name="bodyKindCode">Rigid-body participation mode byte.</param>
    /// <param name="useGravity">True when gravity should be enabled.</param>
        /// <returns>Serialized rigid-body component payload.</returns>
        byte[] WriteRigidBody3DComponentPayload(byte bodyKindCode, bool useGravity) {
            using MemoryStream stream = new MemoryStream();
            using EngineBinaryWriter writer = EngineBinaryWriter.Create(stream, EngineBinaryEndianness.LittleEndian);
            writer.WriteByte(1);
