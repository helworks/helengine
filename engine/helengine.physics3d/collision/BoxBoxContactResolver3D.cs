...
namespace helengine {
    ...
    public static class BoxBoxContactResolver3D {
        ...
        public static bool TryResolveContact(BodyState3D first, BodyState3D second, out float penetration, out int axisIndex) {
            ...
            float xPenetration = PrimitiveContactMath3D.CalculateAxisPenetration(first.Position.X, first.HalfExtents.X, second.Position.X, second.HalfExtents.X);
            float yPenetration = PrimitiveContactMath3D.CalculateAxisPenetration(first.Position.Y, first.HalfExtents.Y, second.Position.Y, second.HalfExtents.Y);
            float zPenetration = PrimitiveContactMath3D.CalculateAxisPenetration(first.Position.Z, first.HalfExtents.Z, second.Position.Z, second.HalfExtents.Z);
