...
namespace helengine {
    ...
    public static class CharacterControllerStaticMeshTriggerResolver3D {
        ...
        /// <param name="currentTriggerPairs">Current step trigger pair set being populated.</param>
        public static void CollectOverlaps(CharacterControllerState3D controllerState, IReadOnlyList<StaticMeshBodyState3D> staticMeshStates, ISet<TriggerPairKey3D> currentTriggerPairs) {
            ...
            for (int index = 0; index < staticMeshStates.Count; index++) {
                ...
                if (!BoxStaticMeshContactResolver3D.TryResolveContact(controllerState.Position, controllerState.HalfExtents, meshState, out _, out _)) {
                    continue;
                }

                currentTriggerPairs.Add(new TriggerPairKey3D(meshState.Entity, controllerState.Entity));
            }
        }
