    ...
    public static class StaticMeshTriggerResolver3D {
        ...
        public static bool TryResolveOverlap(BodyState3D bodyState, StaticMeshBodyState3D meshState) {
            ...
            if (bodyState.ColliderShapeKind == ColliderShapeKind3D.Sphere) {
                ...
                return SphereStaticMeshContactResolver3D.TryResolveContact(bodyState, meshState, out float3 contactNormal, out float penetrationDepth);
#else
                return false;
#endif
            }
            if (bodyState.ColliderShapeKind == ColliderShapeKind3D.Capsule) {
                ...
                return CapsuleStaticMeshContactResolver3D.TryResolveContact(bodyState, meshState, out float3 contactNormal, out float penetrationDepth);
#else
                return false;
#endif
            }
            if (bodyState.ColliderShapeKind == ColliderShapeKind3D.Box) {
                ...
                return BoxStaticMeshContactResolver3D.TryResolveContact(bodyState, meshState, out float3 contactNormal, out float penetrationDepth);
#else
                return false;
#endif
            }
