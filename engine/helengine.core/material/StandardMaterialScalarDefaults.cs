namespace helengine {
    /// <summary>
    /// Packs one authored scalar standard-material value into the sixteen-byte float4 constant-buffer payload shared by the metallic, roughness, and specular defaults.
    /// </summary>
    public static class StandardMaterialScalarDefaults {
        /// <summary>
        /// Creates one packed float4 constant-buffer payload from the supplied scalar value, broadcast into all four components.
        /// </summary>
        /// <param name="value">Authored scalar value that will be clamped to the supported zero-to-one range.</param>
        /// <returns>Sixteen-byte packed constant-buffer payload.</returns>
        [NativeOwnedReturn]
        public static byte[] CreateConstantBufferData(float value) {
