namespace helengine.editor {
    ...
    public class EditorViewportSettingsOverlayComponent : UpdateComponent {
        ...
        /// <summary>
        /// Viewport-local simulated canvas settings edited by this overlay.
        /// </summary>
        ...
        public EditorViewportSettingsOverlayComponent(
            ...
            EditorViewportCanvasPreviewSettings canvasPreviewSettings,
            Action<bool> setGridVisibleAction,
            Func<bool> isGridVisibleResolver) {
            Camera = camera ?? throw new ArgumentNullException(nameof(camera));
            Font = font ?? throw new ArgumentNullException(nameof(font));
            OverlayLayerMask = overlayLayerMask;
            CanvasPreviewSettings = canvasPreviewSettings ?? throw new ArgumentNullException(nameof(canvasPreviewSettings));
        ...
        /// <summary>
        /// Opens the overlay, synchronizes control state from the current viewport settings, and focuses the first control.
        /// </summary>
        public void Open() {
            EnsureInitialized();
        ...
        void CreatePixelsPerWorldUnitRow() {
            ...
            PixelsPerWorldUnitLabelText = new TextComponent {
                Font = Font,
                Text = "Pixels / Unit",
                Color = ThemeManager.Colors.InputForegroundPrimary,
        ...
        /// <summary>
        /// Applies keyboard-focus styling to the pixels-per-world-unit slider target.
        /// </summary>
        /// <param name="isFocused">True when keyboard focus currently targets the pixels-per-world-unit slider.</param>
        void HandlePixelsPerWorldUnitFocusedChanged(bool isFocused) {
            PixelsPerWorldUnitSliderInternal.SetKeyboardFocused(isFocused);
        }
        ...
        void ActivatePixelsPerWorldUnitFromKey(Keys key) {
            if (!CanAdjustSliderWithKey(key)) {
                return;
            }

            PixelsPerWorldUnitSliderInternal.AdjustFromKey(key);
        }
        ...
        /// <summary>
        /// Applies live preview-setting updates from the pixels-per-world-unit slider.
        /// </summary>
        /// <param name="value">New authored pixels-per-world-unit value from the slider.</param>
        void HandlePixelsPerWorldUnitSliderChanged(double value) {
            if (IsSynchronizingState) {
                return;
            }

            CanvasPreviewSettings.PixelsPerWorldUnit = Math.Max(1, (int)Math.Round(value));
            SynchronizeFromState();
        }
        ...
        /// <summary>
        /// Returns true when the supplied point lies inside the pixels-per-world-unit slider.
        /// </summary>
        /// <param name="point">Screen point to evaluate.</param>
        /// <returns>True when the point lies inside the pixels-per-world-unit slider.</returns>
        bool ContainsPixelsPerWorldUnitSliderPoint(int2 point) {
            return ContainsChildPoint(PixelsPerWorldUnitSliderInternal, SliderWidth, SliderHeight, point);
        }
        ...
        /// <summary>
        /// Formats one integer-backed setting value for compact overlay display.
        /// </summary>
        /// <param name="value">Integer value to format.</param>
        /// <returns>Formatted integer string.</returns>
        string FormatIntegerValue(int value) {
