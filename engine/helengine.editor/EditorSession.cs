...
namespace helengine.editor {
    ...
    public class EditorSession {
        ...
        static readonly object UninitializedDisposeGate = new object();
        bool IsDisposed;
        bool IsDisposing;
        int DisposeThreadId;
        ...
        /// <summary>
        /// Optional deterministic construction fault seam used by lifecycle tests.
        /// </summary>
        ...
        public EditorSession(
            ...
            EditorBootTimeline.Mark("authoring: session object");
            assetImportManager.ImportTexturesMissingCache();
            EditorBootTimeline.Mark("authoring: textures missing cache");
            assetImportManager.ImportModelsMissingCache();
            EditorBootTimeline.Mark("authoring: models missing cache");
            ...
            mainViewport.SetInput(core.Input);
            ...
            SceneEntityCreationService = new EditorSceneEntityCreationService(
                sceneAssetReferenceFactory,
                sceneAssetReferenceResolver,
                authoredAssetReferenceResolver,
                generatedAssetProviderRegistry,
                generatedMaterialCache,
                assetImportManager,
        ...
        public void ApplyUiScale(EditorUiScaleSettings settings, EditorUiMetrics metrics, FontAsset uiFont, FontAsset snapModifierFont) {
            ...
            rendererResources.SetDefaultFontAsset(uiFont);
            if (titleBar != null) {
        ...
        void DetachScaleSensitiveDialogHandlers() {
            if (ScaleSensitiveDialogDetacherItems == null) {
                return;
            }
        ...
        void RecreateScaleSensitiveDialogs() {
            ...
            // A partially materialized session used by preference-shell tests has no editor core or
            // project environment service. Keep the already-created preference dialog current while
            // leaving full dialog recreation to the production composition root.
            if (core == null || projectEnvironmentsService == null) {
                preferencesDialog?.ApplyUiMetrics(uiFont, CurrentUiMetrics);
                return;
            }
        ...
        /// </summary>
        public void Dispose() {
            object disposeGate = DisposeGate ?? UninitializedDisposeGate;
            lock (disposeGate) {
                if (IsDisposed) {
                    return;
                }
        ...
        EditorEntity CreateBlueprintInstanceSceneEntity(AssetBrowserEntry entry) {
            return SceneEntityCreationService.CreateBlueprintInstanceSceneEntity(entry, ResolveAddToScenePlacementPosition());
        }
        ...
        /// <summary>
        /// Creates one scene entity for a model asset entry at the current add-to-scene placement position.
        /// </summary>
        ...
        EditorEntity CreateModelSceneEntity(AssetBrowserEntry entry) {
            return SceneEntityCreationService.CreateModelSceneEntity(entry, ResolveAddToScenePlacementPosition());
        }
        ...
        void BuildStartScene() {
            if (rendererResources == null || rendererResources.RenderManager3D == null) {
                throw new InvalidOperationException("Viewport grid initialization requires an active 3D render manager.");
            }

            EditorViewportGridFactory.Create(rendererResources.RenderManager3D, builtInShaderAssetLibrary);
        }
        ...
        void HandleAssetSelected(AssetBrowserEntry entry) {
            ...
            try {
                ...
                if (AssetSelectionCoordinator.IsModelImportSettingsEntry(entry)) {
                    ...
                    for (int index = 0; index < propertiesPanels.Count; index++) {
                        propertiesPanels[index].ShowImportSettings(entry, settings.Importer.ImporterId, settings.Processor, importerIds, SupportedPlatforms, CurrentProjectPlatform, CreateSupportedPlatformDefinitionsById(), ResolveProjectEnvironmentIds());
                    }
                }
                RefreshPreviewSource();
            } catch (Exception ex) {
        ...
        void HandleImportSettingsApplyRequested(AssetBrowserEntry entry, AssetImportSettingsApplyRequest request) {
            ...
            try {
                ...
                if (AssetSelectionCoordinator.IsModelImportSettingsEntry(entry)) {
                    ...
                    for (int index = 0; index < propertiesPanels.Count; index++) {
                        propertiesPanels[index].ShowImportSettings(entry, refreshedSettings.Importer.ImporterId, refreshedSettings.Processor, importerIds, SupportedPlatforms, CurrentProjectPlatform, CreateSupportedPlatformDefinitionsById(), ResolveProjectEnvironmentIds());
                    }
                }
            } catch (Exception ex) {
                IReadOnlyList<PropertiesPanel> propertiesPanels = GetPropertiesPanels();
        ...
        void HandlePropertiesPanelAssetState(PropertiesPanel panel, AssetBrowserEntry entry) {
            ...
            try {
                ...
                if (AssetSelectionCoordinator.IsModelImportSettingsEntry(entry)) {
                    ...
                    assetImportManager.SaveImportSettings(entry.FullPath, settings);
                    panel.ShowImportSettings(entry, settings.Importer.ImporterId, settings.Processor, importerIds, SupportedPlatforms, CurrentProjectPlatform, CreateSupportedPlatformDefinitionsById(), ResolveProjectEnvironmentIds());
                }
            } catch (Exception ex) {
                panel.ShowImportError(entry, ex.Message);
            }
