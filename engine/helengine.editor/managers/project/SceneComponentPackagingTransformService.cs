...
namespace helengine.editor {
    ...
    public sealed class SceneComponentPackagingTransformService {
        ...
        EntityComponentPlatformOverrideState ResolveCommonScopeComponentOverride(SceneComponentAssetRecord persistedRecord) {
            ...
            for (int index = 0; index < overrideStates.Count; index++) {
                EntityComponentPlatformOverrideState overrideState = overrideStates[index];
                if (overrideState != null && overrideState.Scope.IsCommon) {
        ...
        SceneAssetReference RewriteAutomaticComponentReference(Type valueType, SceneAssetReference reference, string buildRootPath) {
            ...
            if (AutomaticComponentAssetReferenceSupport.MatchesSupportedAssetReferenceType(valueType, typeof(AudioAsset))) {
                return RewriteAudioReference(reference, buildRootPath);
            }
        ...
        /// <summary>
        /// Rewrites one authored audio reference into the packaged file-backed reference consumed by runtime scripted component deserializers.
        /// </summary>
        /// <param name="reference">Serialized audio reference to rewrite.</param>
        /// <param name="buildRootPath">Absolute build root path that receives packaged assets.</param>
        /// <returns>Packaged file-backed audio reference.</returns>
        SceneAssetReference RewriteAudioReference(SceneAssetReference reference, string buildRootPath) {
            if (reference == null) {
                throw new InvalidOperationException("AudioAsset-backed component members require an audio reference before packaging.");
            }
            if (reference.SourceKind != SceneAssetReferenceSourceKind.FileSystem) {
        ...
        SceneAssetReference RewriteFileSystemFontReference(SceneAssetReference reference, string buildRootPath) {
            ...
            if (SupportsBuilderOwnedFontAtlasCookKind()) {
                ...
                if (string.Equals(Path.GetExtension(reference.RelativePath), ".hefont", StringComparison.OrdinalIgnoreCase)) {
                    ...
                    RememberGeneratedFontCookWorkItem(generatedAtlasSourceFullPath, cookedAtlasTextureRelativePath, sourceAtlasAssetId, buildRootPath);
                ...
                RememberFontCookWorkItem(fontVariant.CachedAtlasTextureAssetPath, cookedAtlasTextureRelativePath, settings, AssetImportManager.ProjectRootPath);
                return CreateFontFileReference(cookedRelativePath);
            }
        ...
        /// <param name="generatedSourceRootPath">Trusted containing root for the generated texture source.</param>
        ...
        /// <summary>
        /// Records one builder-owned font-atlas cook work item using the supplied generated-source root for source hashing.
        /// </summary>
        /// <param name="sourceAssetPath">Absolute generated atlas source path.</param>
        /// <param name="cookedRelativePath">Runtime-relative cooked atlas output path.</param>
        /// <param name="settings">Resolved authored font import settings.</param>
        /// <param name="generatedSourceRootPath">Trusted containing root for the generated atlas source.</param>
        void RememberFontCookWorkItem(string sourceAssetPath, string cookedRelativePath, AssetImportSettings settings, string generatedSourceRootPath) {
        ...
        /// <param name="generatedSourceRootPath">Trusted containing root for the generated atlas source.</param>
        void RememberGeneratedFontCookWorkItem(string sourceAssetPath, string cookedRelativePath, string sourceAssetId, string generatedSourceRootPath) {
            if (PlatformCookWorkItemSink == null || !SupportsBuilderOwnedFontAtlasCookKind()) {
                return;
            } else if (string.IsNullOrWhiteSpace(sourceAssetPath)) {
                throw new ArgumentException("Source asset path must be provided.", nameof(sourceAssetPath));
