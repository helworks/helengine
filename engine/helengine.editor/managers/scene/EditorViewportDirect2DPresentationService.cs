namespace helengine.editor {
    ...
    public static class EditorViewportDirect2DPresentationService {
        ...
        /// <summary>
        /// Resolves the editor-presented world anchor for one authored selection so editor gizmos align to the visible bounds center of supported 2D content.
        /// </summary>
        /// <param name="entity">Authored scene entity whose presented selection anchor should be resolved.</param>
        /// <returns>Editor-presented world anchor position for the supplied entity.</returns>
        public static float3 ResolvePresentedWorldAnchorPosition(Entity entity) {
            if (entity == null) {
                throw new ArgumentNullException(nameof(entity));
            }

            if (TryGetComponent(entity, out ViewportComponent viewportComponent)) {
                int2 viewportSize = ResolvePresentedWorldSize(entity, viewportComponent);
                return TransformPresentedViewportPoint(entity, new float3(viewportSize.X * 0.5f, viewportSize.Y * 0.5f, 0f));
            }

            if (EditorWorldSpace2DPreviewMapper.TryResolveSupportedSourceComponent(entity, out Component sourceComponent)) {
                int2 previewSize = ResolvePreviewComponentSize(sourceComponent, entity);
                return TransformPresentedEntityLocalPoint(entity, new float3(previewSize.X * 0.5f, previewSize.Y * 0.5f, 0f));
            }

            return ResolvePresentedWorldPosition(entity);
        }
        ...
        /// <summary>
        /// Resolves the stored world position that should be written back for one editor-presented selection anchor point.
        /// </summary>
        /// <param name="entity">Authored scene entity whose presented selection anchor is being edited.</param>
        /// <param name="presentedAnchorWorldPosition">Editor-presented world-space anchor position chosen by scene-view interaction.</param>
        /// <returns>Stored world-space position that preserves the authored/runtime coordinate contract behind the presented anchor.</returns>
        public static float3 ResolveStoredWorldPositionFromPresentedAnchor(Entity entity, float3 presentedAnchorWorldPosition) {
            if (entity == null) {
                throw new ArgumentNullException(nameof(entity));
            }

            float3 presentedOriginPosition = ResolvePresentedOriginPositionFromAnchor(entity, presentedAnchorWorldPosition);
            return ResolveStoredWorldPositionFromPresented(entity, presentedOriginPosition);
        }
        ...
        /// <summary>
        /// Resolves the presented origin position that corresponds to one presented selection anchor.
        /// </summary>
        /// <param name="entity">Authored scene entity whose origin should be reconstructed from a presented anchor.</param>
        /// <param name="presentedAnchorWorldPosition">Presented anchor world position.</param>
        /// <returns>Presented origin world position that maps back to the authored/runtime stored coordinate contract.</returns>
        static float3 ResolvePresentedOriginPositionFromAnchor(Entity entity, float3 presentedAnchorWorldPosition) {
            if (TryGetComponent(entity, out ViewportComponent viewportComponent)) {
                int2 viewportSize = ResolvePresentedWorldSize(entity, viewportComponent);
                float3 presentedAnchorOffset = TransformPresentedViewportPoint(entity, new float3(viewportSize.X * 0.5f, viewportSize.Y * 0.5f, 0f)) - ResolvePresentedWorldPosition(entity);
                return presentedAnchorWorldPosition - presentedAnchorOffset;
            }

            if (EditorWorldSpace2DPreviewMapper.TryResolveSupportedSourceComponent(entity, out Component sourceComponent)) {
                int2 previewSize = ResolvePreviewComponentSize(sourceComponent, entity);
                float3 presentedAnchorOffset = TransformPresentedEntityLocalPoint(entity, new float3(previewSize.X * 0.5f, previewSize.Y * 0.5f, 0f)) - ResolvePresentedWorldPosition(entity);
                return presentedAnchorWorldPosition - presentedAnchorOffset;
            }

            return presentedAnchorWorldPosition;
        }
        ...
        /// <param name="component">Receives the first matching component when present.</param>
        /// <returns>True when one component of the requested type was found; otherwise false.</returns>
        static bool TryGetComponent<T>(Entity entity, out T component) where T : Component {
            if (entity == null) {
                throw new ArgumentNullException(nameof(entity));
            }

            for (int componentIndex = 0; componentIndex < entity.Components.Count; componentIndex++) {
                if (entity.Components[componentIndex] is T resolvedComponent) {
