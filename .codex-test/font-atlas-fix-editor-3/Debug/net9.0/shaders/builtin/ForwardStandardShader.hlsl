...
cbuffer EmissiveColorBuffer : register(b7)
{
    float4 emissiveColorValue;
};
...
float4 PS(PS_IN input) : SV_Target
{
    ...
    float3 emissiveTextureSample = EmissiveTexture.Sample(EmissiveTextureSampler, input.texCoord).rgb;
    float hasEmissiveTexture = materialFlags.y;
    float emissiveStrength = emissiveColorValue.a;
    float3 emissiveColor = hasEmissiveTexture > 0.5f
        ? emissiveTextureSample * (emissiveStrength > 0.0001f ? emissiveColorValue.rgb * emissiveStrength : float3(1.0f, 1.0f, 1.0f))
        : emissiveColorValue.rgb * emissiveStrength;
