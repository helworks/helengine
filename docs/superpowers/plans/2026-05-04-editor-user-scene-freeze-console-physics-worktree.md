# Editor User Scene Freeze Implementation Plan (Console Physics Worktree)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Freeze user-authored scene updates and authored scene interactivity inside the editor after the `console-physics-architecture` merge, while keeping editor-owned tools and UI responsive and leaving `helengine.core` untouched.

**Architecture:** Add one editor-only suppression service in `helengine.editor` that scans user scene roots and removes user-owned `IUpdateable` and `IInteractable2D` registrations from the live `ObjectManager` before each editor update. Then wire `EditorSession.Update(double elapsedSeconds)` to invoke that service before `core.Update(elapsedSeconds)`, and prove the behavior with direct service tests plus one editor-session integration test that matches the future timing-aware update path.

**Tech Stack:** C# / .NET 9, HelEngine editor/runtime libraries, console-physics worktree code shape, xUnit, existing editor test harnesses in `engine/helengine.editor.tests`
...
- Modify: `engine/helengine.editor/EditorSession.cs`
  - Add a suppression-service field and invoke it from `Update(double elapsedSeconds)` so authored scene activity is frozen before the timing-aware core update runs.
...
- Create: `engine/helengine.editor.tests/EditorSessionRuntimeSuppressionIntegrationTests.cs`
  - Validate that `EditorSession.Update(double elapsedSeconds)` suppresses user scene runtime activity before the main core update advances authored components.

## Task 1: Add Direct Suppression Tests For The Future Branch
...
Expected: `FAIL` because `EditorSceneRuntimeSuppressionService` does not exist yet in the merged worktree.
...
## Task 2: Wire Suppression Into The Timing-Aware Editor Update Path
...
Expected: `FAIL` because `EditorSession.Update(double elapsedSeconds)` still calls `core.Update(elapsedSeconds)` directly, so the authored `FPSComponent` updates its visible text.

- [ ] **Step 3: Initialize and invoke the suppression service from `EditorSession.Update(double elapsedSeconds)`**
...
## Task 3: Run Focused Regression Verification On The Merged Branch
